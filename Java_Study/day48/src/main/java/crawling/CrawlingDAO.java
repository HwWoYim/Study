...
public class CrawlingDAO {
	...
	// Crawling data
	public boolean insert(CrawlingVO vo) {
	...
	public boolean update(CrawlingVO vo) {
		conn = JDBCUtil.connect();
		try {
	...
	// 차트 출력
	// selectAll 이므로 배열리스트 output, CrawlingVO vo input
	public ArrayList<CrawlingVO> selectAll(CrawlingVO vo) {
		// 차트를 받아오기 위해 JDBC연결
		conn = JDBCUtil.connect();
		ArrayList<CrawlingVO> datas = new ArrayList<CrawlingVO>();
		try {
			// stmt(Statement)
			...
			while (rs.next()) {
				CrawlingVO data = new CrawlingVO();
	...
	public ArrayList<CrawlingVO> selectAll_title(CrawlingVO vo) {
		// 차트를 받아오기 위해 JDBC연결
		conn = JDBCUtil.connect();
		ArrayList<CrawlingVO> datas = new ArrayList<CrawlingVO>();
		try {
		...
		// 선언한 배열 datas에 저장
		return datas;
	} public ArrayList<CrawlingVO> selectAll_artist(CrawlingVO vo) {
		// 차트를 받아오기 위해 JDBC연결
		conn = JDBCUtil.connect();
		ArrayList<CrawlingVO> datas = new ArrayList<CrawlingVO>();
		try {
	...
	// 노래 검색
	// 노래 들 중 하나를 선택하는 것이라 output CrawlingVO 선택
	public CrawlingVO selectOne(CrawlingVO vo) {
		conn = JDBCUtil.connect();
		try {
	...
	// 샘플데이터가 있는지?
	public boolean hasSample(CrawlingVO vo) {
		conn = JDBCUtil.connect();
		try {
